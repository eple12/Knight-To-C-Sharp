...
public static class Bitboard
{
    ...
    public static void Highlight(ulong[] bitboard, int bitboardIndex, GameObject highlight)
    {
        string bitsString = BitsString(bitboard[bitboardIndex]);
        for (int i = 0; i < 64; i++)
        {
            if (bitsString[i] == '1')
            {
                highlight.transform.GetChild(i).gameObject.SetActive(true);
            }
            else
            {
